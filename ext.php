...
class ext extends base
{
	...
	public function is_enableable()
	{
		...
		$user->add_lang_ext('avathar/bbguildffxiv', 'info_ext');

		if (version_compare(PHP_VERSION, self::MIN_PHP_VERSION, '<'))
		{
			$errors[] = $user->lang('BBGUILDFFXIV_PHP_VERSION_FAIL', self::MIN_PHP_VERSION, PHP_VERSION);
		}

		if (phpbb_version_compare(PHPBB_VERSION, self::MIN_PHPBB_VERSION, '<'))
		{
			$errors[] = $user->lang('BBGUILDFFXIV_PHPBB_VERSION_FAIL', self::MIN_PHPBB_VERSION, PHPBB_VERSION);
		}

		$ext_manager = $this->container->get('ext.manager');
		if (!$ext_manager->is_enabled('avathar/bbguild'))
		{
			$errors[] = $user->lang('BBGUILDFFXIV_REQUIRES_BBGUILD');
		}
